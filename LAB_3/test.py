...
# Load image for img0
img0 = cv2.imread("datasets/face1.jpg")
...
while cap.isOpened():
    ...
    matched_keypoints_0 = np.float32(
        [keypoints_0[m.queryIdx].pt for m in matches]
    ).reshape(-1, 1, 2)
    matched_keypoints_cam = np.float32(
        [keypoints_cam[m.trainIdx].pt for m in matches]
    ).reshape(-1, 1, 2)

    # Find homography matrix
    H, _ = cv2.findHomography(matched_keypoints_0, matched_keypoints_cam, cv2.RANSAC)
    ...
    cv2.polylines(frame, [np.int32(transformed_corners)], True, (0, 255, 0), 2)
